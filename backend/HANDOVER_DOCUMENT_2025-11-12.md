...
**交接日期**: 2025-11-12
**项目状态**: P1阶段进行中 (25% 完成度)
**系统状态**: 🟡 运行中（房间分配算法重写进行中）
**维护者**: Claude Code → 你（AI 工程师）
...
#### 🆕 2025-11-12：房间分配重构指引（供接手的 AI 工程师使用）

- **配置来源统一**：所有中心/期次配置读取 `src/main/resources/config`，运行时不再访问 `backend/config/*.yaml`。`session.elderly_age_threshold` 优先，其次中心配置字段，最后默认 60 岁。
- **Excel → Java 行为映射**：
  | Excel 宏 | Java 必须实现的逻辑 |
  | --- | --- |
  | `CheckFriend` / `SplitFellow` | `CompanionSplitter`：在男女数组上拆散同伴（旧生↔旧生，新生按年龄互换），保持与 VBA 完全一致。 |
  | `CBedInfo.getUsableRooms` | `RoomQueueBuilder`：按 `room_type + gender_area` 生成法师/旧生/新生/老人1/老人2 队列，并对房间+床位执行 `shuffle`。老人1/2 需通过房间字段或 notes 标记。 |
  | `BedTemp_A/B` | `assignStudents()`：固定顺序分配（法师→旧生→新生→老人1→老人2），房间耗尽则记录未分配学员。 |
  | `DisruptBed` | 通过 `Collections.shuffle` + `RoomCursor` 打散队列及房内床位顺序。 |
- **实现步骤**：
  1. 新增 `RoomQueueBuilder`、`RoomCursor`、`CompanionSplitter` 辅助类，放在 allocation 相关包。
  2. 在 `AllocationServiceImpl.autoAllocate()` 中：读取阈值 → 构建房间队列 → 拆散同伴 → 男女分别调用 `assignStudents()` → 写入 `Allocation(roomId, bedNumber)`。
  3. `AllocationResult` 返回 `elderlyThreshold`、`unassignedCount`、`unassignedStudents`、`perGenderStats`，供前端显示。
  4. 单元/集成测试覆盖：房型耗尽、老人 fallback、同伴拆散、男女/单性课程。至少用一份 Excel 导出的数据核对房号与宏输出一致。
