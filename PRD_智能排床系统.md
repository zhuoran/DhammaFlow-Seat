...
**当前实现（2025-11-29 SeatAllocator.java）**：
- 按 sectionName 分组后再按性别拆组分配，reserved 格子在入口即剔除，容量按实际可坐格子数计算（避免非矩形布局的虚假空位）。
- 排序：法师 > 修学次数降序 > 年龄降序；法师作为旧生参与旧生段。
- 分配顺序（每 section）：OLD_STUDENT 专区预分 → 旧生前两行 row-major → 旧生继续到倒数第二行 → 若旧生在行内耗尽，用新生从右向左补满该行 → 新生竖列右→左、上→下（保留最后一行）→ 最后一行先旧生后新生 → 补空座（available）。
- 性别未标记的 section 记录警告并默认归入女区；后续优化需在配置中补全性别标记。
...
**用户故事**:
> 作为排床专员，根据学员优先级和房间类型，系统自动将他们标记为同一组。
