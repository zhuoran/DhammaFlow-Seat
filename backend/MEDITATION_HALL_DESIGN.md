...
### 6.2 座位查看 & 调整（当前实现）
- 双区视图：女众左、男众右，底部对齐；空行补齐保证最后一排对齐。
- 卡片字段：座号/姓名，房间床位（带区域前缀）、年龄（纯数字）、课次、服务次；颜色按性别、左侧色条凸显区域。
- 交互：点击卡片查看详情；顶部支持按姓名/座号/房间床位快速定位并高亮；打印模式隐藏操作区、统一浅色底。
- 后续计划（未实现）：交换座位、释放座位、同伴分离报告。
